    ...
    (list
     (org-contextualize-keys (delq nil
	  (mapcar
	   ...
	   custom-commands)) org-agenda-custom-commands-contexts)
     prefix-descriptions)))
...
(if (featurep 'transient)
	...
	[:description
	 ...
	 :pad-keys t]))
  (defun org-agenda-transient ()
    "Display a transient UI for Org agenda commands."
    (interactive)
    (user-error "Transient is not available")))
